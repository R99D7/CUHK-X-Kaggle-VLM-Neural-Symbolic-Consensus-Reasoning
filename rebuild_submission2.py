...
Fixing the sequence actions parsing.
"""
import pandas as pd
...
# Build sequence actions (PROPERLY SPLITTING COMMA-SEPARATED STRINGS)
vid_to_seq_acts = {}
for idx, row in te[te['category'] == 'sequence'].iterrows():
    vid = row['path']
    seq_str = str(row['A']).strip().lower()  # Just need one option to get the 4 actions
    acts = set([a.strip() for a in seq_str.split(',')])
...
sub.to_csv('submission_v262_FIXED2.csv', index=False)
