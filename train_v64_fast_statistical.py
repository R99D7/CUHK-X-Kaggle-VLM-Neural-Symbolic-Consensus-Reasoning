...
def get_exact_substring(s1, s2):
    return 1 if str(s2).lower() in str(s1).lower() else 0
...
def extract_fast_features(df):
    ...
    for idx, row in df.iterrows():
        ...
        for opt in ['A', 'B', 'C', 'D']:
            ...
            is_sub = get_exact_substring(q, ans)
            ...
            # 4. Length Ratio
            ratio = a_len / (q_len + 1e-5)
            
            row_feats.extend([a_len, a_words, jac, is_sub, overlap, ratio])
            
        features.append(row_feats)
    return np.array(features)

def run_fast_statistical():
    ...
    print("Extracting Fast Statistical Features...", flush=True)
    X_train_stat = extract_fast_features(train_single)
    X_test_stat = extract_fast_features(test_single)
    ...
    print("Training Fast Stacking Classifier...", flush=True)
    base_models = [
        ('hgb1', HistGradientBoostingClassifier(max_iter=150, learning_rate=0.05, l2_regularization=0.1, random_state=42)),
        ('hgb2', HistGradientBoostingClassifier(max_iter=150, learning_rate=0.1, max_leaf_nodes=15, random_state=43))
    ]
    ...
    out.to_csv('submission_v64_fast_statistical.csv', index=False)
