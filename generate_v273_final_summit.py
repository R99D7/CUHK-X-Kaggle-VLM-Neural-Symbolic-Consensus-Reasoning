"""
Generate submission_v273_FINAL_SUMMIT.csv by applying 26 surgical prunes of uncorroborated,
low-confidence (< 0.35 probability) action noise over the 0.70760 baseline.
...
for vid_path, grp in grouped:
    ...
    if noisy_letters and len(retained_letters) > 0:
        new_pred = "".join(sorted(retained_letters))
        if new_pred != m_pred:
            sub.loc[sub['qa_id'] == qid, 'prediction'] = new_pred
            changes_count += 1
            print(f"[v273 SURGICAL CLEANUP] {qid}: {m_pred} -> {new_pred} (Pruned uncorroborated noise: {noisy_letters})")

assert len(sub) == 682, f"Expected 682 rows, found {len(sub)}"
print(f"\nSuccessfully generated submission_v273_FINAL_SUMMIT.csv with {changes_count} surgical multi-noise prunes!")

sub.to_csv("submission_v273_FINAL_SUMMIT.csv", index=False)
sub.to_csv("submission.csv", index=False)
print("Saved to submission_v273_FINAL_SUMMIT.csv and submission.csv!")
