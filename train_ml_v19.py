...
def run_ml_pipeline_v11():
    ...
    vectorizer = TfidfVectorizer(ngram_range=(1, 5), analyzer='word', max_features=100000, sublinear_tf=True)
    ...
    # Create column names
    num_cols = ['cat_idx', 'letter_idx', 'q_len', 'opt_len', 'len_diff', 'len_ratio', 'word_overlap', 'jaccard']
    ...
    gbm = HistGradientBoostingClassifier(
        ...
        categorical_features=['cat_idx', 'letter_idx'], random_state=42, class_weight='balanced', l2_regularization=0.1
    )
    ...
    print("Saving submission_ml_v19_mpnet.csv...")
    pd.DataFrame(final_preds).to_csv('submission_ml_v19_mpnet.csv', index=False)
    print("Done!")
