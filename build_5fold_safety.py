...
final_pred_df['prob_A'] = final_pred_df[[f'prob_A_f{i}' for i in range(5)]].mean(axis=1)
final_pred_df['prob_B'] = final_pred_df[[f'prob_B_f{i}' for i in range(5)]].mean(axis=1)
final_pred_df['prob_C'] = final_pred_df[[f'prob_C_f{i}' for i in range(5)]].mean(axis=1)
final_pred_df['prob_D'] = final_pred_df[[f'prob_D_f{i}' for i in range(5)]].mean(axis=1)
...
pd.DataFrame(final_res).to_csv('submission_v32_5fold_safety.csv', index=False)
print("Saved 5-Fold Ensembled Predictions to submission_v32_5fold_safety.csv")
