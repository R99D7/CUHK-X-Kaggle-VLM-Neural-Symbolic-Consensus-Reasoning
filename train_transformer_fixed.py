...
class VideoQATransformerDataset(Dataset):
    ...
    def _prepare_data(self):
        ...
        for idx, row in tqdm(self.data.iterrows(), total=len(self.data)):
            ...
            # THE FIX: Add _Depth.pt
            vid_feat_path = os.path.join(self.video_feature_dir, f"{vid_id}_Depth.pt")
...
def train():
    ...
    torch.save(model.state_dict(), 'cnn_transformer_fixed.pth')
    print("Model saved to cnn_transformer_fixed.pth")
    ...
    pd.DataFrame(all_preds).to_csv('transformer_fixed_raw_predictions.csv', index=False)
    print("Saved transformer_fixed_raw_predictions.csv!")
