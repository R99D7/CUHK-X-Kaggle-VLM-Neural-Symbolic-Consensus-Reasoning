...
v9 = pd.read_csv('submission_ml_v9.csv')
moondream = pd.read_csv('submission_final.csv')

v9_dict = dict(zip(v9['qa_id'], v9['prediction']))
...
for qa_id in cat_dict:
    cat = cat_dict[qa_id]
    pred_v9 = v9_dict.get(qa_id, 'A')
    pred_md = md_dict.get(qa_id, 'A')
    
    # Moondream2 is much better at visual tasks like emotion/object interaction.
    # Text ML V9 (XGBoost/CatBoost) is heavily optimized for structural logic (sequence, combination, multi).
...
ensemble_df.to_csv('submission_ensemble_v10.csv', index=False)
print("Ensemble saved to submission_ensemble_v10.csv!")
