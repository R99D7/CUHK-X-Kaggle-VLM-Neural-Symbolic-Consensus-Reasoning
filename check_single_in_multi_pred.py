"""
Check if any single option is predicted in multi.
"""
import pandas as pd
...
for idx, row in te[te['category'] == 'single'].iterrows():
    ...
    # Are any of the SINGLE options in the MULTI *prediction*?
    opts_in_multi = [l for l, txt in opts.items() if txt in multi_acts]
    
    if len(opts_in_multi) == 1 and pred != opts_in_multi[0]:
        print(f"SINGLE {row['qa_id']}: pred {pred} ({opts.get(pred, 'nan')}) -> change to {opts_in_multi[0]} ({opts[opts_in_multi[0]]}) (predicted by MULTI)")
        changes += 1

print(f"\nTotal SINGLE fixes from MULTI prediction: {changes}")
