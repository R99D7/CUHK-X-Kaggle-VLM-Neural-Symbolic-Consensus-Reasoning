...
for idx, row in tqdm(test_df.iterrows(), total=len(test_df)):
    ...
    # FIX: Use the actual path from the CSV
        ...
        if expected_len == 1:
            ...
            if match:
                ...
                for l, opt in [('A', row['A']), ('B', row['B']), ('C', row['C']), ('D', row['D'])]:
                    if str(opt).lower() in answer.lower():
                        pred = l
                        break
                if not pred:
                    for c in ans_up:
...
out.to_csv('submission_v41_moondream_fixed.csv', index=False)
print(f'Done! Saved submission_v41_moondream_fixed.csv. Fallbacks used: {failed_count}')
