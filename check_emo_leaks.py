"""
Check emotion exact leaks against Moondream and Submission.
"""
import pandas as pd
...
moon_dict = dict(zip(moon['qa_id'], moon['prediction']))
...
for idx, row in te.iterrows():
    if row['category'] != 'emotion': continue
    ...
    if sig in tr_sigs:
        ...
        if len(train_ans_texts) == 1:
            ...
            if correct_letter:
                ...
                if moon_dict.get(row['qa_id']) == correct_letter: moon_correct += 1

print(f"Total emotion exact leaks: {total_leaks}")
print(f"Submission correct: {sub_correct}/{total_leaks}")
print(f"Moondream correct: {moon_correct}/{total_leaks}")
