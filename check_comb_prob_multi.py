"""
Check COMB->MULTI additions using raw probability > 0.5
"""
import pandas as pd
...
for idx, row in te[te['category'] == 'multi'].iterrows():
    ...
    if missing:
        print(f"MULTI {row['qa_id']} (vid {vid}): pred {multi_pred} missing {missing} from COMB {comb_q['qa_id']} (prob {p:.4f})")
        changes += 1

print(f"\nTotal MULTI fixes from confident COMB: {changes}")
