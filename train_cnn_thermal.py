...
class VideoQATransformerDataset(Dataset):
    ...
    def _prepare_data(self):
        ...
        for idx, row in tqdm(self.data.iterrows(), total=len(self.data)):
            ...
            # THE FIX: Add _Thermal.pt
            vid_feat_path = os.path.join(self.video_feature_dir, f"{vid_id}_Thermal.pt")
...
def train():
    ...
    torch.save(model.state_dict(), 'cnn_thermal.pth')
    print("Model saved to cnn_thermal.pth")
    ...
    pd.DataFrame(all_preds).to_csv('cnn_thermal_raw_predictions.csv', index=False)
    print("Saved cnn_thermal_raw_predictions.csv!")
