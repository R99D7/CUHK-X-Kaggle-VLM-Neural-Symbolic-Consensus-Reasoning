...
for i, model_preds in disagreements:
    ...
    # Try forcing this answer to be RIGHT
    z[i, c].lowBound = 1
    z[i, c].upBound = 1
    ...
    if pulp.LpStatus[prob.status] == 'Infeasible':
        ...
        feasible_options = []
        # ONLY check options that AT LEAST ONE MODEL PREDICTED!
        # Because if no model predicted it, changing to it doesn't affect the score equations!
        # Wait, if changing to it doesn't affect the equations, the solver would say it's feasible
        # if the remaining score equations can be satisfied by other variables.
        # But we only want to pick an answer if it's the ONLY feasible one among plausible options.
